    ...
    try {
      ...
      if(data.success){
       ...
       toast.error(data.message)
        ...
        console.log('Toggle car availability error:',error);
        toast.error(error.response?.data?.message || 'Failed to toggle car availability');
      } 
  }
    ...
    <div className='px-4 pt-10 md:px-10 w-full'>
        ...
        <div className='max-w-3xl w-full rounded-md overflow-hidden border border-borrderColor mt-6'>
           <table className='w-full border-collapse text-left text-sm to-gray-600'>
                 ...
                 <tbody>
                     {cars.map((car,index)=>(
                      <tr key={index} className='border-t border-borrderColor '>
                          <td className='p-3 flex items-center gap-3 '>
                             ...
                             {/* category */}
                           <td className='p-3 max-md:hidden text-gray-500'>{car.category}</td>
                           {/* Price */}
                          <td className='p-3 text-gray-500'> {currency}{car.pricePerDay}/Day</td>
